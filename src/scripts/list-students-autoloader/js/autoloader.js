...
require(['underscore'], function(_) {
  ...
  var saveButtonTemplate = $j($j('#new-button-template').html());
  var saveSubmit = $j('#saveSubmit');
  ...
  $j.when($j.get('/ws/schema/table/U_DEF_AUTOLOADER_LISTS?q=usersdcid==' + psData.userDCID + '&projection=*'), $j.get('/ws/schema/table/U_DEF_GLOBAL_LISTS?projection=*'))
    .done(function(userReportsData, globalReportsData) {
      ...
      $j('#saveSubmit').on('click', function(e) {
        ...
        $j.get('/admin/studentlist/enable_requests.html?frn=204' + psData.userDCID, function(enableResp) {
          $j.ajax({
            data: encodedData,
            type: 'POST',
      ...
      $j('#newSubmit').on('click', function(e) {
        e.preventDefault();
        loadingDialogInstance.open();
        var formObj = serializeFormToObject();
        var formData;
